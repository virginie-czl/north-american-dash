...
import { zipStored, uniqueNames } from "./zip.ts";
...
const collided = uniqueNames([
  { name: "Le Balcon.csv", text: "a" },
  { name: "Le Balcon.csv", text: "b" },
  { name: "le balcon.csv", text: "c" },
  { name: "Other.csv", text: "d" },
]);
t(
  "a repeated name is numbered, not dropped",
  collided.map((e) => e.name).join(" | ") ===
    "Le Balcon.csv | Le Balcon (2).csv | le balcon (3).csv | Other.csv",
  collided.map((e) => e.name).join(" | "),
);

// ── The archive itself, read back by the system unzip ───────────────────────
const entries = [
  { name: "Espace Canal — CA-2411-0847.csv", text: "Poste;Montant\nDû;8 200,00\n" },
  { name: "Traiteur Agnus Dei — CA-2411-0847.csv", text: "Poste;Montant\nDû;4 120,00\n" },
];
...
try {
  ...
  t("the file carries a BOM for Excel", first.charCodeAt(0) === 0xfeff);
} catch (error) {
