...
/**
 * Reads the channel and returns every card approval seen.
 *
 * Paginates back through history; the channel is high-volume so this is capped.
 * Results are cached per instance for a few minutes — approvals are append-only,
 * so slight staleness is harmless and it keeps the Slack API calls low.
 */
const CACHE_TTL_MS = 5 * 60_000;
let cache: { at: number; approvals: CardApproval[] } | null = null;

export async function fetchCardApprovals(): Promise<CardApproval[]> {
  if (cache && Date.now() - cache.at < CACHE_TTL_MS) return cache.approvals;
