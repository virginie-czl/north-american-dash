...
export const Route = createFileRoute("/_authenticated/veolia")({
  ...
  head: () => ({
    meta: [
      { title: "Veolia — Invoicing SLA Tracker" },
      {
        name: "description",
        content:
          "Veolia free-invoicing deals in North America and/or sold by Malika, Mathieu or Mara.",
      },
    ],
  }),
  component: SlaPage,
...
function invoicingSla(
  ...
  if (sentDate) {
    ...
    return { label: `Sent late ${lateBy}d`, variant: "overdue" };
  }
  const now = Date.now();
  ...
  if (emission) {
    ...
    return { label: `Issued, send breached ${overBy}d`, variant: "overdue" };
  }
  if (now <= deadline) {
    const daysLeft = Math.max(0, Math.ceil((deadline - now) / 86_400_000));
...
function SlaPage() {
  ...
  const kpis = useMemo(() => {
    ...
    decorated.forEach(({ row }) => {
      ...
      // Live service fees from liveConfirmed (gross - net GMV HT), independent of invoicing.
      const fee = row.live_service_fees_ht ?? 0;
                              ...
                              <TableCell className="whitespace-nowrap">
                                {r.purchase_order_number ? (
                                  <>
                                    ...
                                    <div className="cell-sub">
                                      since {fmtDateShort(r.purchase_order_date)}
                                    </div>
                                  </>
                                ) : (
